...
enrichmentator<-function(x,y,z){
	...
	matriz_nombres_genes <- read.table("lib/matriz_nombres_genes.csv", row.names = 1, header=TRUE, quote="\"", stringsAsFactors=FALSE)
	...
	hypgeoResults_kegg <- multiHyperGeoTest(collectionOfGeneSets= GS_KEGG, universe= universo, hits= COMUNIDAD_ENTREZ, minGeneSetSize = 15, pAdjustMethod = "BH")
	...
	#escribir_tabla
	nombre_kegg<-paste0(z,"comunidad_kegg_", nombre_comunidad)
	...
	hypgeoResults_GS_GO_MF <- multiHyperGeoTest(collectionOfGeneSets= GS_GO_MF, universe= universo, hits= COMUNIDAD_ENTREZ, minGeneSetSize = 15, pAdjustMethod = "BH")
	...
	hypgeoResults_GS_GO_BP <- multiHyperGeoTest(collectionOfGeneSets= GS_GO_BP, universe= universo, hits= COMUNIDAD_ENTREZ, minGeneSetSize = 15, pAdjustMethod = "BH")
	...
	hypgeoResults_GS_GO_CC <- multiHyperGeoTest(collectionOfGeneSets= GS_GO_CC, universe= universo, hits= COMUNIDAD_ENTREZ, minGeneSetSize = 15, pAdjustMethod = "BH")
